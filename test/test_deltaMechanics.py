...
class TestDeltaMechanics(unittest.TestCase):
    ...
    def test_possible_position(self):
        ...
        self.assertAlmostEqual(dut.theta[0], np.deg2rad(-20.5), 2)
        self.assertAlmostEqual(dut.theta[1], np.deg2rad(-20.5), 2)
        self.assertAlmostEqual(dut.theta[2], np.deg2rad(-20.5), 2)
        self.assertEqual(dut.valid, True)

        dut.update_from_new_tip_pos(np.array([0.3, 0.5, -1.1]))
        self.assertAlmostEqual(dut.theta[0], np.deg2rad(47.5), 2)
        self.assertAlmostEqual(dut.theta[1], np.deg2rad(-11.6), 2)
        self.assertAlmostEqual(dut.theta[2], np.deg2rad(21.4), 2)
        self.assertEqual(dut.valid, True)
    ...
    def test_speed_computation(self):
        ...
        self.assertEqual(dut.valid, True)
        theta_t0 = dut.theta.copy()

        dut.update_from_new_tip_pos(pos_t1)
        self.assertEqual(dut.valid, True)
        theta_t1 = dut.theta.copy()
