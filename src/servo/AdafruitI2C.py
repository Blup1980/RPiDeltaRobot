...
class AdafruitI2C(object):
        ...
        try:
            with open('/proc/cpuinfo', 'r') as infile:
                for line in infile:
                    ...
                    # info in front of the revision (like 1000 when the Pi was over-voltage).
    ...
    def __init__(self, address, busnum=-1, debug=False):
        ...
        self.bus = smbus.SMBus(busnum if busnum >= 0 else AdafruitI2C.get_pi_i2c_bus_number())
        self.debug = debug
    ...
    def write_list(self, reg, list_in):
        """Writes an array of bytes using I2C format"""
        try:
            if self.debug:
                print("I2C: Writing list to register 0x%02X:" % reg)
                print(list_in)
            self.bus.write_i2c_block_data(self.address, reg, list_in)
        except IOError:
            return self.err_msg()
